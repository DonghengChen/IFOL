...
local notation "cneg" => (Sum.inr 3)
local notation "cimpl" =>  (Sum.inr 4)
local notation "cforall" => (Sum.inr 5)
local notation "cexists" =>  (Sum.inr 6)
local notation "cbot" =>  (Sum.inr 0)
...
| ⟨cneg,fn⟩ => fopt1 (finv (fn ⟨0, by decide⟩)) Formula.negation
