...
notation "Fterm" n => (Term.free (free_variable.free_variable n))
...
@[simp]
theorem size_of_substit_eq_depth {f : Formula σ} : ∀ m e, (f.substitution m e).depth = f.depth := by
  induction f <;> (intro m e;cases m) <;> first | rfl | simp; aesop

def Term.free_variables {σ : Signature} : Term σ  → Set ℤ
| free x =>  match x with
            | .free_variable z =>if z ≥ 0 then {z} else ∅
| const _ =>  ∅

def Formula.free_variables {σ : Signature} : Formula σ  → Set ℤ
  | atomic_formula r ts => ⋃ (i:Fin (σ.arity' r)), (ts i).free_variables
  | ¬ᵢ f => free_variables f
  | ⊥   => ∅
  | f1 ∧ᵢ f2 | f1 ∨ᵢ f2 | f1 →ᵢ f2 =>f1.free_variables ∪ f2.free_variables
  | ∃ᵢ f | ∀ᵢ f => f.free_variables

def Set.free_variables (Γ : Set (Formula σ)) : Set ℤ :=
  ⋃ (f ∈ Γ), f.free_variables
...
| introF (A)(Γ)(x) : Proof Γ A → x ∉ (Set.free_variables Γ) → x≥0 → Proof Γ (∀ᵢ (A.substitution  (.free (IFOL.free_variable.free_variable x)) (.free (IFOL.free_variable.free_variable (-A.depth)))))
| elimF  (A)(Γ)(τ: Term σ) : Proof Γ (∀ᵢ A) → Proof Γ (A.substitution (.free (free_variable.free_variable (-A.depth))) τ)
| introE (A)(Γ)(t: Term σ) : Proof Γ A → Proof Γ (∃ᵢ A.substitution t (.free (free_variable.free_variable (-A.depth))))
| elimE (A B)(Γ)(τ: Term σ): Proof Γ (∃ᵢ A) → Proof (Q ∪ {A.substitution (Fterm (-A.depth)) τ }) B → ((τ.free_variables) ∩ (A.free_variables)=∅ ) → Proof (Γ ∪ Q) B --fix lift
...
def modify_value_function (M : model σ) (v : Term σ → M.A) (item : M.A) (index : ℤ): Term σ → M.A --change the item_mapping of specfic variable
| .free z => match z with
  | .free_variable n => if n=index then item else v (.free (.free_variable (n)))
| .const z => v (.const z)
...
| f1 ∨ᵢ f2 => (f1.force_form  M w hw v) ∨ (f2.force_form  M w hw v)
| ∃ᵢ f => ∃ (t:M.A), f.force_form M w hw (modify_value_function M v t (-f.depth))
| ∀ᵢ f => ∀ (t:M.A), f.force_form M w hw (modify_value_function M v t (-f.depth))
...
lemma Formula.mono_proof {σ : Signature}(M: model σ)(u v:M.world)(hr: M.R u v)(g: Formula σ)(hw: u ∈ M.W)(val:Term σ → M.A)(h1: g.force_form M u hw val ): g.force_form M v (M.R_closed u v hr hw) val:=
  ...
  | existential_quantification f =>
    let ⟨t,ht⟩ := h1
    ⟨t,(Formula.mono_proof M u v hr f hw (modify_value_function M val t (-f.depth)) ht)⟩
  | universal_quantification f => fun t =>
    Formula.mono_proof M u v hr f hw (modify_value_function M val t (-f.depth)) (h1 t)



-- lemma subForm_subV_eq{σ : Signature}{f:Formula σ}{M: model σ }{u:M.world}{hw: u ∈M.W}
-- {val:Term σ → M.A}(h0:f.force_form M u hw val)(n m: ℤ)(b:M.A):
-- (f.substitution (@Term.free σ (free_variable.free_variable n))
--  (@Term.free σ (free_variable.free_variable m))).force_form M u hw (modify_value_function M val b m):=match f with
--  | .atomic_formula r ts => by
--     unfold Formula.force_form at h0
--     unfold Formula.force_form
--     simp [Formula.substitution]
--     unfold modify_value_function
--     dsimp


-- lemma sub_iso_sub {σ : Signature}{f:Formula σ}{M: model σ }{u:M.world}{hw: u ∈M.W}{v:Term σ → M.A}
-- (h0:f.force_form M u hw v)(n m: ℤ)(obj:M.A):(f.substitution  (Fterm n) (Fterm m) ).force_form M u hw (modify_value_function M v obj m):=
-- match f with
-- | .atomic_formula r ts =>
...
-- | ¬ᵢ f => by sorry
-- | f1 →ᵢ f2 => by sorry
-- | f1 ∧ᵢ f2 => by simp at h0;simp;constructor;apply sub_iso_sub h0.left;apply sub_iso_sub h0.right
-- | f1 ∨ᵢ f2 => by simp at h0;simp;cases h0;left;apply sub_iso_sub;assumption ;right;apply sub_iso_sub;assumption
-- | ∃ᵢ f => by simp at h0
--              simp
--              rcases h0 with ⟨t,ht⟩
--              use t
...
def public_value_eql {σ : Signature}{M : model σ}(v:Term σ → M.A)(v2:Term σ → M.A) : Prop:=
∀(t: Term σ), match t with
| .free a=> match a with
  | .free_variable x=> if x < 0 then True else (v (Term.free (free_variable.free_variable x))) = (v2 (Term.free (free_variable.free_variable x)))
| .const a=> match a with
  | .Constant x=> (v (Term.const (Constant.Constant x))) = (v2 (Term.const (Constant.Constant x)))

theorem value_ref  {σ : Signature}{M : model σ}(v:Term σ → M.A) : public_value_eql v v :=by
unfold public_value_eql
intro t
simp
cases t with
| free a => simp
...
∀ (M : model σ), ∀ (w : M.world), ∀ (v : Term σ → M.A)(hw),∃(v2 : Term σ → M.A), (∀ (f :Formula σ ),f ∈ Γ → f.force_form  M w hw v) → (public_value_eql v v2) → A.force_form  M w hw v

notation Γ "⊧" A => semantic_consequence Γ A

def prop_semantic {σ : Signature} {Γ : Set (Formula σ)} {A : Formula σ} :
(∀ (M : model σ), ∀ (w : M.world), ∀ (v : Term σ → M.A)(hw), (∀ (f :Formula σ ),f ∈ Γ → f.force_form  M w hw v) → A.force_form  M w hw v )→ Γ ⊧ A:=by
intro cond
intros M w v hw
use v
have conds := cond M w v hw
intro s _
exact conds s

def soundness {σ  : Signature} {Q : Set (Formula σ)} {A : Formula σ} (h : Q ⊢ A) : (Q ⊧ A) :=
match h with
| Proof.ref h => prop_semantic  _
...
| Proof.elimI  A B Q X hb hc =>fun M u v hw =>
...
-- | Proof.introF B Γ x hx h1 h2 => fun M u v hw hf =>by
--  unfold Formula.force_form
--  simp [Formula.substitution,size_of_substit_eq_depth]
--  intro t

--  have h3:_:= soundness hx M u
--  unfold semantic_consequence at h3
--  have h4:_:= h3 v hw hf
--  apply sub_iso_sub h4
